...
parser.add_argument("--dt", help="updating step length/ms", type=float, default=20)
...
parser.add_argument("--t_on", help="Input duration of one component", type=int, default=20)
parser.add_argument('--Dt_on', help='Variability of Input timing', type=int, default=10)
parser.add_argument('--t_item', help='Duration of one component', type=int, default=60)

parser.add_argument('--t_delay', help="delay duration", type=int, default=40)
parser.add_argument('--Dt_delay', help='Variability of delay duration', type=int, default=40)
parser.add_argument("--t_retrieve", help="Retrieve duration of one component", type=int, default=40)

parser.add_argument("--t_cue", help="Go cue signal duration", type=int, default=10)
parser.add_argument("--t_ron", help="One time window length of retrieval", type=int, default=10)
parser.add_argument("--t_fix", help="fixation time window between two component of retrieval", type=int, default=10)
parser.add_argument("--t_final", help="resting time period before the end of one trial", type=int, default=40)
...
parser.add_argument("--freq_report", help="frequency/epochs of reporting training loss", type=int, default=100)
parser.add_argument("--freq_save", help="frequency/epochs of saving models", type=int, default=200)
args = parser.parse_args()

# Network construction
P = dict(train_hidden_0=False, act_func=args.act_func, in_Channel=args.in_Channel, out_Channel=args.out_Channel,
         dt=args.dt, tau=args.tau, g_in=args.g_in, g_rec=args.g_rec,
         N_Neuron=args.N_Neuron, t_on=args.t_on, Dt_on=args.Dt_on, t_item=args.t_item,
         t_delay=args.t_delay, Dt_delay=args.Dt_delay, t_retrieve=args.t_retrieve,
         t_cue=args.t_cue, t_ron=args.t_ron, t_fix=args.t_fix, t_final=args.t_final
         )
...
# batchiy mixed sequence trainset and training
...
for epoch in range(args.n_epochs):
    ...
    for Batch_seq in mixed_trainset:
        tl, l_r, l_f, l_p = train_NOREVERSE(RNN, optimizer, scheduler, Batch_seq, default_Vector, w_reg=args.w_reg,
                                            weight=args.loss_weight, device=device, regtype=args.regtype)
        Btl += tl
        Bl_r += l_r
        Bl_f += l_f
        Bl_p += l_p
    ...
    if epoch % args.freq_report == args.freq_report - 1:
        ...
        print('This Epoch takes:{} seconds.\nThe whole process takes:{} seconds'.format(str(end-start),str(end-start0)))
    if epoch % args.freq_save == args.freq_save - 1:
        torch.save(RNN, out_dir + '//model_' + str(save_count) + '.pth')
        save_count += 1

# After training,save loss file
np.savez(out_dir + '//loss.npz', Loss=np.array(Etl), Loss_r=np.array(El_r), Loss_f=np.array(El_f),
