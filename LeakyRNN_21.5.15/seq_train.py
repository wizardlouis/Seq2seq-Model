...
def train_w_reg(Net, optimizer, scheduler, Batch_Seq, Vectorrep, device='cpu', **kwargs):
    ...
    l_ar = len(Batch_Seq) * activity_reg_loss(Net.act_func(hidden), regtype=kwarg['aregtype'])
    l_r = len(Batch_Seq) * reg_loss(Net, w_reg=kwarg['w_reg'], regtype=kwarg['regtype'])
    l_rW = len(Batch_Seq) * reg_loss_W(Net, w_reg=kwarg['w_reg'], regtype=kwarg['regtype'], **kwarg)
    l_f = fixed_loss(param, out)
    l_p = pos_loss(param, out, Batch_Seq, Vectorrep, device=device, reverse=False)
    weight = kwarg['weight']
...
def train_ortho_proj(Net, optimizer, scheduler, Batch_Seq, Vectorrep, device='cpu', **kwargs):
    ...
    tl = weight[0] * l_h0 + weight[1] * l_ar + weight[2] * l_r + weight[3] * l_f + weight[4] * l_p
    tl.backward()
    if 'In_Wz' and 'In_z' in kwarg.keys():
        Net.In.weight.grad=(kwarg['In_Wz'].to(device))@Net.In.weight.grad@(kwarg['In_z'].to(device))
    if 'W_Wz' and 'W_z' in kwarg.keys():
        Net.W.weight.grad=(kwarg['W_Wz'].to(device))@Net.W.weight.grad@(kwarg['W_z'].to(device))
    if 'Out_Wz' and 'Out_z' in kwarg.keys():
        Net.Out.weight.grad=(kwarg['Out_Wz'].to(device))@Net.Out.weight.grad@(kwarg['Out_z'].to(device))
    torch.nn.utils.clip_grad_norm_(Net.parameters(), 1.)
    optimizer.step()
    if scheduler is not None:
        scheduler.step(tl)
