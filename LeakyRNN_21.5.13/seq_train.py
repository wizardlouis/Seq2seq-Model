...
def train(Net, optimizer, scheduler, Batch_Seq, Vectorrep, input_type='reproduct', w_reg=1, weight=[1, 1, 1, 1],
          device='cpu',
          aregtype='L1', regtype='L2', strength=10,init_hidden='default'):
    ...
    if init_hidden=='default':
        hidden, out = Net(Input,device=device)
    elif init_hidden=='rand':
        hidden, out = Net(Input,hidden_0=nn.Parameter(torch.randn(Net.N).to(device), requires_grad=False), device=device)
    else:
        print('init_hidden keyword missed or wrong!!!')
    l_ar = len(Batch_Seq) * activity_reg_loss(Net.act_func(hidden), regtype=aregtype)
