...
def regularization_loss(Net,w_reg):
    ...
    L_W=Loss(Net.RW.weight,torch.zeros_like(Net.RW.weight))
    L_R=Loss(Net.Geometry.weight,torch.zeros_like(Net.Geometry.weight))
    return w_reg*(L_E+L_R)+L_W

def regularization_loss_fixed(Net):
    Loss=nn.MSELoss(reduction='mean')
    L_W = Loss(Net.W, torch.zeros_like(Net.W))
    return L_W

#geo(Batch_Size,T,2)
def fixed_loss(Net,geo,Batch_seq):
    P=Net.P;L_seq=len(Batch_seq[0])
    t_retrieve=P['t_rrest']+(L_seq-1)*P['t_rinterval']+P['t_ron']+P['n_windows']+10
    geo_d=geo[:,:-t_retrieve]
    geo_cue=geo[:,-t_retrieve:-t_retrieve+P['t_cue']]
    geo_f=geo[:,-5:]
    Loss=nn.MSELoss(reduction='sum')
    return Loss(geo_d,torch.zeros_like(geo_d))/(len(geo[0])-t_retrieve)+Loss(geo_cue,torch.zeros_like(geo_cue))/P['t_cue']+Loss(geo_f,torch.zeros_like(geo_f))/5

#geo(Batch_Size,T,2),Batch_Seq(Batch_Size,L_seq)
#用这种形式得到的一个epoch的loss_maximum=sum(num_batch*L_seq)比如：1*1+3*2+8*3
def pos_loss(Net,geo,Batch_Seq,Direction,device='cpu'):
    L_seq = len(Batch_Seq[0])
    P=Net.P;t0=-(L_seq-1)*P['t_rinterval']-P['t_ron']-P['n_windows']-10;dt=P['t_rinterval'];dt_=P['t_ron']+P['n_windows']-1
    Slides=torch.tensor([],requires_grad=True)
    total_pos_loss=0
    #将每个seq上item重复dt_次，然后转换成二维坐标(Batch_Size,L_seq*dt_,2)
    target=torch.tensor(Batch_Seq).reshape(-1,1).repeat(1,dt_).reshape(-1,L_seq*dt_).to(device)
    target=torch.tensor([[Direction[i-1] for i in Seq] for Seq in target]).to(device)
    ...
    Window=torch.zeros(dt_,P['n_windows'],requires_grad=False).to(device)
    for i in range(P['n_windows']):
        Window[i:i+P['t_ron'],i]=1
    #按照窗口求平均的diff矩阵(Batch_Size,L_seq,n_windows)
    Diff_Window=(Diff@Window/P['t_ron']).to('cpu')
...
def train(Net,Net_optimizer,scheduler,Batch_seq,w_reg,weight=[1,1,1],device="cpu",fixed=False):
    Net_optimizer.zero_grad()
    hidden_0=Net.reset_hidden()
    Input=Batch_Seq2Input(Batch_seq,Net.Vectorrep,Net.Vec_embedding,Net.P,device=device)
    hidden,geo=Net(hidden_0,Input,device=device)
    if not fixed:
        l_r=len(Batch_seq)*regularization_loss(Net,w_reg)
    else:
        l_r=len(Batch_seq)*regularization_loss_fixed(Net)
    l_f=fixed_loss(Net,geo,Batch_seq)
    l_p=pos_loss(Net,geo,Batch_seq,Net.Direction,device)
...
def iter_train(Net,Net_optimizer,scheduler,trainset,batch_size,repeat_size,w_reg,weight=[1,1,1],n_epochs=200,device="cpu",fixed=False):
    Etl,El_r,El_f,El_p=[],[],[],[]
    for epoch in range(n_epochs):
        ...
        for Batch_seq in mixed_trainset:
            tl,l_r,l_f,l_p=train(Net,Net_optimizer,scheduler,Batch_seq,w_reg,weight,device=device,fixed=fixed)
