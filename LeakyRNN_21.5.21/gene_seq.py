...
def p2p_REPRODUCT(net_param, L_seq, add_param=None):
    ...
    # pointer to fix_on
    pointer = 0
    pointer += N['fix_loss_on']
    param.update({'fixl_b1': pointer})
    pointer += N['fix_on']
    param.update({'fix_b1': pointer})
...
def Batch_Seq2Input(Batch_Seq, Vectorrep, P, strength=10, type='reproduct'):
    ...
    elif type=='reproduct_symbreak':
        return Batch_Seq2Input_REPRODUCT_SYMBREAK(Batch_Seq,Vectorrep,P,strength=strength)
...
#For act_func such as tanh whose symmetry is odd, we need extra input channel to manipulate unique stable state during relaxing
def Batch_Seq2Input_REPRODUCT_SYMBREAK(Batch_Seq,Vectorrep,P,strength=10):
    Batch_Size=len(Batch_Seq)
    L_seq=len(Batch_Seq[0])
    Vector=torch.tensor(Vectorrep)
    Input=torch.zeros(Batch_Size,P['total'],P['in_Channel'])
    for seq_id in range(Batch_Size):
        for item_id in range(L_seq):
            Input[seq_id,P['t_b'+str(item_id)]:P['t_e'+str(item_id)],:2]=strength*Vector[Batch_Seq[seq_id][item_id]-1].unsqueeze(dim=0).repeat(P['t_on'],1)
    Input[:,P['fix_b1']:P['fix_e1'],2]=1.*strength
    Input[:,:P['fix_b1'],3]=1.*strength
    Input[:,P['fix_e1']:,3]=1.*strength
    return Input
