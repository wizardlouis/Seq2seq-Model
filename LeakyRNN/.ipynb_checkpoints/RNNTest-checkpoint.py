...
Loss,CELoss,scaleLoss=Iter_train_burn(l_RNN,optimizer,None,trainset,n_epochs,4,burn_in=burn_in)
...
Loss=np.array(Loss);CELoss=np.array(CELoss);scaleLoss=np.array(scaleLoss)

score_train=Batch_testNet_True(l_RNN,trainset,n_item)
score_test=Batch_testNet_True(l_RNN,testset,n_item)
...
np.savez(out_dir+'//loss.npz',Loss=Loss,CELoss=CELoss,scaleLoss=scaleLoss)
