...
def Batch_Seq2Input(Batch_Seq,Vectorrep,Vector_embedding,P,add_delay=None):
    ...
    if add_delay==None:
        T=P['t_rest']+L_seq*P['t_on']+(L_seq-1)*P['t_off']+P['t_delay']+P['t_retrieve']+random.randint(P['ad_min'],P['ad_max'])
    else:
        T=P['t_rest']+L_seq*P['t_on']+(L_seq-1)*P['t_off']+P['t_delay']+P['t_retrieve']+add_delay
    ...
    #cue信号的位置
    temporal[:,-P['t_retrieve']:-P['t_retrieve']+P['t_cue'],0]=1
    for i in range(Batch_Size):
        for j in range(L_seq):
            temporal[i,P['t_rest']+j*(P['t_on']+P['t_off'])+Vary[i,j]:P['t_rest']+j*(P['t_on']+P['t_off'])+P['t_on']+Vary[i,j],j+1]=1
    axis=Vector_embedding(torch.tensor([0,1,2]))
    Input=torch.einsum('abc,adb->adc',(Batch_Vec,temporal))@axis
    return Input
...
def balanced(Seqs, n_item):
    ...
    avg = round(Batch_Size / n_item);std = math.ceil(avg * 0.05)
