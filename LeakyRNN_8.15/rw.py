...
def plot_trajectory2(Net,Batch_Seq,row,column=5):
    ...
    geometry = geometry.detach().numpy()
    geometry=geometry[-Net.t_retrieve-Net.t_delay : -Net.t_retrieve+Net.t_cue+seq_length*Net.t_ron+(seq_length-1)*Net.t_roff]
    c=np.zeros(len(geometry))
    for i in range(seq_length):
        c[Net.t_delay+Net.t_cue+i*(Net.t_ron+Net.t_roff):Net.t_delay+Net.t_cue+i*(Net.t_ron+Net.t_roff)+Net.t_ron]=1
    cm=plt.cm.get_cmap('bwr')
    for k in range(len(Batch_Seq)):
        plt.subplot(row,column,k+1)
        plt.xlim(-2, 2)
