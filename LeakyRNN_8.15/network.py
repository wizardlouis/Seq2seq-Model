...
class myRNN(nn.Module):
    def __init__(self,N_Neuron,W,g_D,act_func,Vec_embedding,Vectorrep,
        ...
        self.Vec_embedding=Vec_embedding;self.Vectorrep=Vectorrep;self.g_D=g_D
        self.t_rest = t_rest;self.t_on = t_on;self.t_off = t_off;self.t_ron=t_ron;self.t_roff=t_roff
        self.t_delay = t_delay;self.t_retrieve = t_retrieve;self.t_cue = t_cue
        self.training=training
        self.use_scale_loss=False;self.train_geometry=train_geometry
    ...
    def forward(self,hidden_0,Batch_Seq):
        ...
        for frame in range(T):
            hidden=(1-self.d)*hidden+self.d*(torch.einsum('ab,cb->ca',[self.W,self.act_func(hidden)])+Input[:,frame,:])
    ...
    def getTarget(self,trial,n_item):
        return torch.cat([trial[-self.t_retrieve+self.t_cue+i*(self.t_ron+self.t_roff):
                                  -self.t_retrieve+self.t_cue+self.t_ron+i*(self.t_ron+self.t_roff)] for i in range(n_item)],dim=0)
...
class lowrank_RNN(nn.Module):
    def __init__(self,N_Neuron,N_rank,left_Vector,right_Vector,g_C,g_D,act_func,Vec_embedding,Vectorrep,
        ...
        self.t_rest=t_rest;self.t_on=t_on;self.t_off=t_off;self.t_delay=t_delay;self.t_retrieve=t_retrieve;self.t_cue=t_cue
    ...
    def forward(self,hidden_0,Batch_Seq):
        ...
        hidden=hidden_0.unsqueeze(dim=0).repeat(Batch_Size,1)
        for frame in range(T):
            hidden=(1-self.d)*hidden+self.d*(torch.einsum('ab,cb->ca',[W,self.act_func(hidden)])+Input[:,frame,:])
            hidden_t=torch.cat((hidden_t,self.act_func(hidden).unsqueeze(dim=0)),dim=0)
        geometry=self.Geometry(self.act_func(hidden_t))
        readout=self.Readout(geometry)
        return hidden_t,geometry,readout
    ...
    def getTarget(self,readout,n_item):
        return torch.cat([readout[-self.t_retrieve+self.t_cue+i*(self.t_on+self.t_off):
                                  -self.t_retrieve+self.t_cue+self.t_on+i*(self.t_on+self.t_off)] for i in range(n_item)],dim=0)
    ...
    def get_param(self):
        dic={'N_Neuron':self.N,'N_rank':self.r,'left_Vector':self.l_Vector,'right_Vector':self.r_Vector,
             'g_C':self.g_C,'g_D':self.g_D,'act_func':self.act_func,'Vec_embedding':self.Vec_embedding,'Vectorrep':self.Vectorrep,
             't_rest':self.t_rest,'t_on':self.t_on,'t_off':self.t_off,'t_delay':self.t_delay,'t_retrieve':self.t_retrieve,'t_cue':self.t_cue,
             'Vector_train':self.Vector_train,'unscaled_noise_M':self.unscaled_noise_M,'noise_Train':self.noise_Train,'decay':self.d}
        return dic
